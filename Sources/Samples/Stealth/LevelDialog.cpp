	...
	UI::Dialog()
{
	...
	UI::Label* labelY = new UI::Label();
	labelY->SetPosition(PointInt(5, top+=20));
	labelY->SetSize(SizeInt(80, 20));
	labelY->SetText(_T("Depth:"));
	Children().Add(labelY);
	...
	UI::TextBox* textBoxDepth = new UI::TextBox();
	textBoxDepth->SetName("textBoxDepth");
	textBoxDepth->SetPosition(PointInt(80, top+=20));
	textBoxDepth->SetSize(SizeInt(80, 20));
	textBoxDepth->SetText("4");
	Children().Add(textBoxDepth);
	...
	_TextBoxDepth = (UI::TextBox*)GetChildByName("textBoxDepth");
...
void LevelDialog::Show(UI::Widget* owner)
{
...
bool LevelDialog::OnButtonOKClick(Object* sender, const EventArgs& e)
{
	...
	int32 depth = _TextBoxDepth->GetText().ToInt32();

	GameCore::Instance()->GetWorld()->NewMap(x, y, depth);
