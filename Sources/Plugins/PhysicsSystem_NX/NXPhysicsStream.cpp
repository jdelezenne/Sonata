...
NxStream& MemoryWriteBuffer::storeBuffer(const void* buffer, NxU32 size)
	...
	if(expectedSize > maxSize)
		...
		if(data)
			{
			Memory::Copy(newData, data, currentSize);
			delete[] data;
			}
		data = newData;
		}
	Memory::Copy(data+currentSize, buffer, size);
	currentSize += size;
	return *this;
	}
...
NxU8 MemoryReadBuffer::readByte() const
	{
	NxU8 b;
	Memory::Copy(&b, buffer, sizeof(NxU8));
	buffer += sizeof(NxU8);
	return b;
	}
...
NxU16 MemoryReadBuffer::readWord() const
	{
	NxU16 w;
	Memory::Copy(&w, buffer, sizeof(NxU16));
	buffer += sizeof(NxU16);
	return w;
	}
...
NxU32 MemoryReadBuffer::readDword() const
	{
	NxU32 d;
	Memory::Copy(&d, buffer, sizeof(NxU32));
	buffer += sizeof(NxU32);
	return d;
	}
...
float MemoryReadBuffer::readFloat() const
	{
	float f;
	Memory::Copy(&f, buffer, sizeof(float));
	buffer += sizeof(float);
	return f;
	}
...
double MemoryReadBuffer::readDouble() const
	{
	double f;
	Memory::Copy(&f, buffer, sizeof(double));
